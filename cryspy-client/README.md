...
* [`src/cryspy_client/styles.cljs`](src/cryspy_client/styles.cljs): CSS compilation source file (ClojureScript,
[Garden](https://github.com/noprompt/garden))
* [`src/cryspy_client/`](src/cryspy_client/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/cryspy_client/core.cljs): contains the SPA entry point, `init`
* [`test/cryspy_client/`](test/cryspy_client/): test files (ClojureScript,
...
5. Clone this repo and open a terminal in the `cryspy-client` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/cryspy_client/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns cryspy-client.example
  (:require [cryspy-client.config :as config])
